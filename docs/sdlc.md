...
- `quint-fpf` — `[pair]` — First Principles Framework. Six phases: `q0-init` → `q1-hypothesize` / `q1-add` → `q2-verify` → `q3-validate` → `q4-audit` → `q5-decide`. Structured reasoning with audit trail and bias checks. Support commands: `q-actualize` (reconcile with repo state), `q-decay` (evidence freshness), `q-query` (search KB), `q-reset` (reset cycle), `q-status` (show status).

...
parse_repositories() {
    # Emits one TSV line per active repository: name<TAB>path
...
for r in cfg.get("repositories", []):
    if r.get("status", "active") != "active":
        continue
    print("\t".join([r.get("name",""), r.get("path","")]))
PY
    else
...
write_header "NONoise multi-repo - Pull All"
...
while IFS=$'\t' read -r name path; do
    ...
    if [ ! -d "$full_path/.git" ]; then
        write_warn "$name - not a git repository (skip): repos/$path"
        skipped=$((skipped + 1))
        continue
    fi

    write_info "$name - pulling..."
    if (cd "$full_path" && git pull --ff-only >/dev/null 2>&1); then
        write_success "$name - up to date"
        pulled=$((pulled + 1))
    else
        write_err "$name - pull failed"
        errors=$((errors + 1))
        error_names+=("$name")
    fi
...
echo ""
[ "$pulled"  -gt 0 ] && write_success "Pulled:  $pulled"
...
echo ""
if [ "$errors" -eq 0 ]; then
    printf "${C_GREEN}All active repositories are up to date.${C_RESET}\n"
fi
echo ""

...
Builds (or refreshes) the workspace's knowledge graph at `graphify-out/` by invoking the **`/graphify` slash skill**. Always uses the IDE assistant's own model — never the headless `graphify extract` CLI — so no external LLM API key is required.
...
By default `/index .` indexes the workspace root, which means **across all sub-repos under `repos/`**. If you want a per-repo graph instead, pass the explicit path: `/index repos/<sub-repo-name>`.

## Usage

```
/index                          # indexes the workspace root (all sub-repos)
/index repos/<sub-repo>         # indexes a single sub-repo

...
description: Produces and incrementally maintains a versioned reverse-engineering dossier for a specific subject — a legacy codebase, a current microservice, a third-party API, an internal library, a data pipeline, anything worth understanding. Output is a collection of markdown chapters under `docs/support/reverse/<subject-slug>/` built by combining project documents (sprint inputs, meeting notes, functional specs) with the subject's code/artifacts indexed via the [graphify](../graphify-setup/SKILL.md) knowledge graph. The flow is (1) graphify indexing — loads context only, writes nothing; (2) long-running interactive Q&A loop on the graph and source; (3) **explicit save trigger from the user** ("save", "ok write it", "make v1.5") bumps the minor version, updates only the chapters in the consolidated intent, appends a changelog entry. Use when the user asks to "reverse-engineer X", "document legacy Y", "build a dossier on vendor Z's API", "refresh the reverse on <subject>", "new version of the reverse", "add what emerged from this meeting to the reverse of <subject>", or mentions a path under `docs/support/reverse/` in an update context. Do NOT use for one-shot graph queries (route to `/graphify query`) or for single-chapter manual edits outside a versioning flow.

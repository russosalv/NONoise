...
description: Meta-skill that coaches the AI + user pair through ANY operational task (deploy, pipeline, provision, teardown, rollback, migrate, scale, cron, diagnose, debug-wiring) in ANY stack, then crystallizes the successful workflow into a project-local reusable skill. Starts by asking WHAT the user wants to achieve, then proposes an ordered access menu (CLI > API > Web) to reach the target environment, gathers context from the project's `docs/architecture/` + optional graphify index, proposes a dry-run plan with risk annotations, executes in paired or delegated mode, and — on success — invokes `skill-creator` to save the exact command sequence as a project-local skill under `.claude/skills/<slug>/SKILL.md`. Use this skill whenever the user says "deploy this service", "create a CI/CD pipeline for X", "set up rollback", "provision dev env", "run this migration", "I need to wire correlation-ID tracking into our observability layer so the AI can debug prod", "automate this ops thing I do manually", "I have credentials — help me operate on my cloud", "ops on Azure / AWS / GCP / kubernetes / any cloud", or when the user wants to turn a manual operation into a repeatable team skill. Also trigger without explicit mention when the user describes a repeated manual ops workflow and the conversation has no generic deploy skill that would fit their stack — this is the right skill because it builds THEIR deploy skill.
